...
public class SmartPhone {
	...
	// 배열에 인스턴스를 저장하고, 수정하고, 삭제, 저장된 데이터의
	// 리스트를 출력하는 메소드를 정의합니다.

	private Contact[] contacts; // Contact 타입의 인스턴스를 저장하는 배열 선언 -> 즉, 참조변수이다.
	private int cnt; // 저장된 Contact 타입의 인스턴스 개수, 배열의 index로 사용

	public static Scanner sc = new Scanner(System.in); // 계속 쓰여지는 변수라면 static 을 붙여서 쓰면 좋다
	...
	private int searchIndex(String name) {
		...
		// 배열의 요소를 반복해서 참조하고 이름과 비교 -> 찾으면 반복 종료 index 리턴
		for (int i = 0; i < cnt; i++) {
	...
	public void editContact() {
		...
		if (index < 0) {
			...
			System.out.println("연락처 등록을 시작합니다.");
			System.out.println("이름 >>");
			String ename = sc.nextLine(); // 위의 지역변수 name 과 중복됨.
