...
public class ReservationViewer {
	...
	public void manageReservation() {
		try {
			...
			System.out.println("[" + Menu.RESERVATION_SEARCH + "] 예약 정보 검색");
			System.out.println("[" + Menu.EXIT + "] 뒤로 가기");
			int userChoice = inputC.checkUserChoice(" ", Menu.EXIT, Menu.RESERVATION_SEARCH);
			...
			if (userChoice == Menu.INSERT) {
				...
				// 예약 삭제 메소드
				delete();
			} else if (userChoice == Menu.RESERVATION_SEARCH) {
				// 시간으로 예약정보 검색 메소드
				show();
	...
	void show() {
		showAll();
		while (true) {
			String rDate = inputC.validInputEntranceTime("조회할 날짜 및 시간을 입력해주세요. (예: 21-04-30 22)");
