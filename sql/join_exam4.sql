...
from emp
where emp.deptno =
    (select deptno from dept where loc like 'NEW YORK');
...
select m.ename, e.deptno, m.empno
from emp e, emp m
...
select distinct e.ename, e.hiredate, e.job
from emp e, emp m
where e.hiredate <= m.hiredate and m.job = 'MANAGER';
