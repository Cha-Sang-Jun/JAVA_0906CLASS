...
select
    to_char (max (sal) , '9,999,999') as "급여 최고액",
    to_char (min (sal), '9,999,999') as "급여 최저액",
    to_char (sum (sal), '9,999,999') as "급여 총액",
    to_char (round (avg (sal), 1), '9,999,999') as "급여 평균"
    from emp;
...
select job,
    ...
    case
        when deptno = 10 or deptno =20 or deptno = 30 then sum(sal)
        end as 총액
    from emp
group by job, deptno
