...
// 학생의 정보를 넣을 데이터타입을 규정을 하고, 기능은 따로 만들었었다.
...
// 우리가 실행할 프로그램에서는 그 학생 프로그램을 '하나의 변수처럼 다루는 것'이 주된 목표가 된다.
// 즉, 구조체와 기능이 합쳐져서 하나의 온전한 클래스가 만들어지고
// 우리가 필요하면 그 클래스의 변수를 우리 프로그램에서 활용하는 것이 목표이다.
// 즉, 3세대 언어 객체 지향 언어에서는 프로그램을 더 작은 프로그램의 집합으로 보게 되는 것이다.
...
// 객체지향 프로그래밍에서는, 객체 즉 클래스 변수를 만들어서 활용하는게 지향점이다.
// static 키워드가 사용되는 순간, 우리가 객체 지향 프로그래밍이 아니라
// 2세대 언어, 즉 절차 지향적 프로그래밍을 하게 되는 것이다.
// 따라서 우리는 static 키워드의 사용을 최소한으로 해야한다.
// 최종적으로, 객체 선언 없이 사용되는 소수의 메소드/필드를 제외하고서는
// 그 어디에서도 static 키워드는 존재하지 않아야 한다.

// 생성자(Constructor)
// 생성자란 특수한 메소드로써, 해당 클래스의 변수가 초기화 될 때
// 호출되는 메소드이다.
// 만약 해당 클래스 변수를 초기화 할 때 해주어야할 작업(=실행할 코드)이 존재할 시에는
// 생성자를 사용하면 된다.

// 만약 우리가 생성자를 따로 만들지 않을 경우에는
// 자바가 제공해주는 기본 생성자를 사용하게 되는데
// 이 기본 생성자는 해당 객체의 필드값을 기본형 데이터타입이면 0
// 참조형 데이터타입이면 null로 초기화 한다.
...
// 생성자를 우리가 만들어 주면, 더 이상 자바가 제공해주는
// 기본생성자는 사용할 수 없다.

// 또한, 우리가 생성자에 파라미터를 넣어줄 수도 있는데,
// 주의할 점은 만약 파라미터가 있는 생성자만 만들어주면
// 더 이상 파라미터가 없는 생성자는 사용할 수 없다 (예시: Scanner 객체)

// this
// this 키워드는 해당 메소드를 실행시키는 객체 자신을 가리키는 대명사와 같은 키워드이다.
// 이 this 라는 키워드는 우리가 클래스 내부에서 메소드를 만들어 줄 때 자주 사용이 되는데
// 왜냐면 메소드 안에서 파라미터 혹은 변수가 필드와 같은 이름일 경우,
...
public class Ex02Class {
	public static void main(String[] args) {
		...
		// equals() 비교하기
		// equals()의 경우, 우리가 클래스 안에 새로 만들어주지 않으면
		// 부정확한 값이 나온다.
		
		System.out.println("-----------------------------------\n");
		s1 = new Student(4,"ddd", 44, 44, 45);
		s2 = new Student(5,"dd", 44, 44, 46);
		s1.printInfo();
		s2.printInfo();
