...
public class ScannerUtil {
	...
	public static int nextInt(Scanner scanner, String message) {
		...
		// 숫자를 검사해야할 경우에는
		// '^[0-9]+$' 라고 적어주게 된다.
		// ^: 문자열의 시작, [0-9]: 어떤 글자의 패턴인지 지정. 지금은 0부터 9까지의 값만 넣겠다라는 의미
		// +: 글자의 갯수. + 라고 할 시에는 1개 이상을 뜻한다.
		// $: 문자열의 끝
		String pattern = "^[0-9]+$";
		while (!temp.matches(pattern)) { // 해당 스트링이 pattern이라는 파라미터에 들어온 변수와 일치하는지
			System.out.println("숫자만 입력하실수 있습니다.");
			temp = nextLine(scanner, message);
		}

		// 3. 사용자의 입력값을 String 에서 int 로 변환한다.
		// 특정 데이터타입의 값을 다른 값으로 바꿀 때에는
		// 명시적 형변환을 사용해도 되지만,
		// parse 라는 변환 메소드를 실행시킬 수 도 있다.
		// 지금의 경우에는 Integer 클래스의 static 메소드인 parseInt() 를 실행하면 된다.
		int result = Integer.parseInt(temp); // parse에 들어온 스트링 값 temp를 int로 변환
