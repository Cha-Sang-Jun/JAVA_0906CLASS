...
public class MemberViewer {
	...
	public MemberViewer() {
		...
		boardViewer.setMemberViewer(this); // 파라미터에 자기자신이 들어간다
	...
	private void showMenu() {
		...
		String message = new String("1. 회원 정보 수정 2. 회원 탈퇴 3. 뒤로가기");
		while (true) {
			int userChoice = ScannerUtil.nextInt(scanner, message, 1, 3);
			if (userChoice == 1) {
				// 수정 메소드
	...
	private void delete(int id) {
		...
		if (yesNo.equalsIgnoreCase("y")) {
			...
			if (memberController.auth(logIn.getUsername(), password) != null) { // 탈퇴하기전 회원 정보를 확인하는 메소드
				...
				boardViewer.deleteByWriterId(id); // 회원탈퇴 후 게시글, 댓글 지우기 위한 메소드
				logIn = null;
			}
		}
	}
	...
	// 회원 번호를 파라미터로 받아서
	// 해당 회원의 닉네임을 리턴하는 메소드
	public void printNickName(int id) {
		System.out.println(memberController.selectOne(id).getNickname());
	}
