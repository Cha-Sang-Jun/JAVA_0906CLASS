	...
	<main role="main" class="container">
		...
		<div class="my-3 p-3 bg-white rounded shadow-sm">
			<form method="post" enctype="multipart/form-data" action="/hgs/join">
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<c:if test="${not empty emailType.snsType}">
							<input type="email" id="snsId" name="snsId"
								autocomplete="username" value="${emailType.email}"
								placeholder="이메일을 입력해주세요 . *">
	
							<input type="hidden" id="snsType" name="snsType"
								value="${emailType.snsType}">
						</c:if>
				...
				<!-- 패스워드 입력  정규식 활용 -->
				<div class="form-group row">
					<label for="pw" class="col-sm-2 col-form-label"> 비밀번호 </label> <input
						type="password" id="password1" name="password1"
						autocomplete="new-password" placeholder="패스워드를 입력해주세요 . *">
				</div>

				<div class="form-group row">
					<label for="repw" class="col-sm-2 col-form-label"> 비밀번호 </label> <input
				...
				<div id="address" name="address">
					...
					<input type="text" id="roadAddress" placeholder="도로명주소"> <input
				...
				<div>
					<div>성별</div>
					<input type="radio" name="sex" value="0" checked>여자 <input
						type="radio" name="sex" value="1">남자
				</div>
				<div>
					<div>
						중성화 여부<input type="checkbox" id="neutering" name="neutering">
					</div>
				</div>
				<div id="breedContainer">
					<div>품종</div>
					<div>
						<input type="text" id="breed" name="breedIdx" list="breedList"
				...
				<h3>관심사</h3>
				<div id="hashtags">
					<input type="checkbox" name="hashtags" value="28">대형견 <input
						type="checkbox" name="hashtags" value="29">중형견 <input
						type="checkbox" name="hashtags" value="30">소형견 <input
						type="checkbox" name="hashtags" value="8">산책 <input
						type="checkbox" name="hashtags" value="3">강아지 <input
						type="checkbox" name="hashtags" value="17">장난감 <input
						type="checkbox" name="hashtags" value="9">사료 <input
						type="checkbox" name="hashtags" value="1">동물병원 <input
						type="checkbox" name="hashtags" value="5">애견카페 <input
						type="checkbox" name="hashtags" value="2">공원
				</div>
				<div id="joinBtn">text</div>
				<!-- 		<button id="joinBtn">회원가입</button> -->
				<input type="submit" value="회원가입" class="btn  btn-primary">
				<input type="reset" class="btn btn-secondary"> <a
					href="/hgs">로그인창으로 돌아가기</a>
			</form>
		</div>
	</main>
...
<script>
	...
	joinBtn.onclick = function() {
		...
		formData.append('address', $('#roadAddress').val() + "_"
				+ $("#detailAddress").val())
