...
<body>
	...
	<form method="post" enctype="multipart/form-data" action="/hgs/join">
		<c:if test="${not empty emailType.snsType}">
			<input type="email" id="snsId" name="snsId" autocomplete="username" value="${emailType.email}" placeholder="이메일을 입력해주세요 . *"><br>
		...
		<input type="text" id="dogName" name="name" placeholder="반려견 이름을 입력해주세요 . *" ><br>
		<input type="text" id="petNumber" name="petNumber" placeholder="등록번호가 있으시면 입력해주세요! *" ><br>
		...
		<div>
			<div>성별</div>
			<input type="radio" name="sex" value="0" checked>여자
			<input type="radio" name="sex" value="1">남자
		</div>
		<div>
			<div>중성화 여부<input type="checkbox" id="neutering" name="neutering" ></div>
		</div>
		<div id="breedContainer">
			<div>품종</div>
			<div>
				<input type="text" id="breed" name="breedIdx" list="breedList" placeholder="품종을 입력해주세요.">
		...
		<div id="hashtags">	
			...
			<input type="checkbox" name="hashtags" value="17">장난감
			...
			<input type="checkbox" name="hashtags" value="2">공원
		</div>
